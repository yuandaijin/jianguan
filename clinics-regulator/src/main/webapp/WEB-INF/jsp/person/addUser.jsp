...
<div class="g-bd">
	...
	<div><input type="hidden" id="flagId" value="${rolebean.id}" name="flagId"></div>
		...
		<c:if test="${rolebean.id == null}">
			<div class="form-inline">
				 <div class="form-group">
				    <span style="color:red;">*</span><label for="input2" style="letter-spacing:8px;">密码：</label>
				    <input type="password" class="form-control" id="input2" name="password" required="true"  minlength="6" maxlength="16">
				    <em class="errortips"></em>
				  </div>
			</div>
		</c:if>
		<c:if test="${rolebean.id == null}">
			<div class="form-inline">
				 <div class="form-group">
				    <span style="color:red;">*</span><label for="input8">确认密码：</label>
				    <input type="password" class="form-control" id="input8"  name="password2" required="true"  minlength="6" maxlength="16" ispassword="true">
				    <em class="errortips"></em>
				  </div>
			</div>
		</c:if>		
...
$(function(){
	...
	updateOneUserReg.rules[$("#input8").attr('name')] = {
			required:true,
			sensitiveCharacter: true,
			specialCharValidate: true
	    };
	updateOneUserReg.msg[$("#input8").attr('name')] = {
			required:'必填'
	    };
	$('#adduser').validate({
	...
	function getRoles(){
		var url = "<%=path%>/user/queryRoleByid";
		$.ajax({
			type:"get",
			url:url,
			dataType:'json',

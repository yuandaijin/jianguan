var echart = {
    ...
    integrationData: function(data, isSingle, isPercent) {
        var resData = [];
        var color = ['#2A91CC', '#62C483', '#84A2D9'];
        var numArr = [];
                    ...
                    normal: {
                        ...
                        formatter: function(params) {
                            if (isPercent) {
                                var sum = 0;
                                for (var i = 0; i < data.length; i++) {
                                    sum += parseFloat(data[i].data[params.dataIndex]);
                                }
                                if (sum == 0 && data[params.seriesIndex].data[params.dataIndex] == 0) {
                                    ...
                                    for (var i = 0; i < parseInt(numArr.length / num); i++) {
                                        ...
                                        numTotal += numArr[num00];
                                    }
                                    percent = (100 - numTotal).toFixed(1);
                                } else {
    ...
    //多线条表封装开始
    moreLine: function(obj, legendData, xData, contentData, isYPercent, isDataPercent) {
        var myChart = echarts.init(obj);
        option = {
            tooltip: {
                trigger: 'axis'
        ...
        if (isDataPercent) {
            option.label.normal.show = true;
            option.label.normal.position = "top";
            option.label.normal.formatter = '{c} %';
            option.tooltip.formatter = "{a} <br/>{b} : {c} %"
        }
        myChart.setOption(option);
    ...
    getMoreLineData: function(arr) {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                obj.label = {};
                obj.label.normal = {};
                // obj.label.normal.show=true;
                // obj.label.normal.position="top";
                // obj.label.normal.formatter='{c} %';
...
var common = {
    ...
    numPercent: function(num1, num2) {
        ...
        if (num == 0) {
            ...
            return ((numLast / num * 10000) / 100).toFixed(1);
        }
    },
    //计算后面一个数大于前面一个数的百分比开始
    ...
    setQuarter: function() {
            ...
            var myDate = new Date();
            var quaNode1 = (myDate.getMonth() >= 1 && myDate.getMonth() <= 3) ? ($("<option></option>").html("1-3月").attr({ "value": "1,2,3", "selected": "selected" })) : ($("<option></option>").html("1-3月").attr({ "value": "1,2,3" }));
            var quaNode2 = (myDate.getMonth() >= 4 && myDate.getMonth() <= 6) ? ($("<option></option>").html("4-6月").attr({ "value": "4,5,6", "selected": "selected" })) : ($("<option></option>").html("4-6月").attr({ "value": "4,5,6" }));
            var quaNode3 = (myDate.getMonth() >= 7 && myDate.getMonth() <= 9) ? ($("<option></option>").html("7-9月").attr({ "value": "7,8,9", "selected": "selected" })) : ($("<option></option>").html("7-9月").attr({ "value": "7,8,9" }));
            var quaNode4 = (myDate.getMonth() >= 10 && myDate.getMonth() <= 12) ? ($("<option></option>").html("10-12月").attr({ "value": "10,11,12", "selected": "selected" })) : ($("<option></option>").html("10-12月").attr({ "value": "10,11,12" }));

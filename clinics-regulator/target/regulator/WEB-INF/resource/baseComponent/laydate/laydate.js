...
/*封装后的调用方法：
 ------------------*/
/*
<input type="text" class="time" id="start"  onclick="dateTimePick(event,function() {alert('23');})" format="YYYY-MM-DD" max="2099-06-16" istime=false istoday=false relation-start-id="startsd" relation-end-id="end" />
<input type="text" class="time" id="end"  onclick="dateTimePick(event)" format="YYYY-MM-DD" max="2099-06-16" istime=false istoday=false  relation-start-id="start" />
<input type="text" class="time" id="hh"  onclick="dateTimePick(event)" format="YYYY-MM-DD" max="2099-06-16" istime=false istoday=false />
min: 函数体内代码字符串 （如：min="return laydate.now();"）
max: 函数体内代码字符串 （如：max="return laydate.now();"）
onclick: dateTimePick(event || event, function);接受一个或两个参数，event：event对象；function：回调函数，它被传入一个对象，这个对象有两个属性：thisEl保存着对这个input的引用，datas保存着当前选中的日期字符串。
relation-start-id 关联的开始时间选择的input的id
relation-end-id 关联的结束时间选择的input的id
*/
function ParametFn(ele, callback) {
  function getAttr(attr) {
    try {
      return [attr, ele.attributes[attr].nodeValue];
    } catch(ex) {
      return '';
    }
  }
  this.elem = "#" + ele.id;
  this.format = getAttr('format')[1] ? getAttr('format')[1] : 'YYYY-MM-DD';
  this.min = this.min ? this.min : (getAttr('min')[1] ? (new Function(getAttr('min')[1]))() : '');
  this.istime = getAttr('istime')[1] == 'true' ? true : false; //thisEle.getAttribute('istime')获取的值为string类型
  this.max = this.max ? this.max : (getAttr('max')[1] ? (new Function(getAttr('max')[1]))() : '2099-06-16');
  this.istoday = getAttr('istoday')[1] == 'true' ? true : false;
  var relationStart = getAttr('relation-start-id')[0] ? getAttr('relation-start-id')[0].split('-')[2] : '';
  var relationEnd = getAttr('relation-end-id')[0] ? getAttr('relation-end-id')[0].split('-')[2] : '';
    this.choose = function(datas){
      if(relationStart) {//结束时间选择--关联的时间选择的类型是start时（开始时间），说明此时间选择是结束时间
        if(!paramet[getAttr('relation-start-id')[1]]) {
          paramet[getAttr('relation-start-id')[1]] = {};
        }
        paramet[getAttr('relation-start-id')[1]].max = datas; //结束日选好后，充值开始日的最大日期
      }
      if(relationEnd) { //开始时间选择
        if(!paramet[getAttr('relation-end-id')[1]]) {
          paramet[getAttr('relation-end-id')[1]] = {};
        }
        paramet[getAttr('relation-end-id')[1]].min = datas; //开始日选好后，重置结束日的最小日期
        paramet[getAttr('relation-end-id')[1]].start = datas //将结束日的初始值设定为开始日
      }
...
var paramet = {};
function dateTimePick(event, callback) {
  var thisEle = event.target || window.event;
  if(!paramet[thisEle.id]) {
    paramet[thisEle.id] = {};
  }
  ParametFn.call(paramet[thisEle.id], thisEle, function(date) {
	  var obj = {
			  datas: date,
			  thisEle: thisEle
	  };

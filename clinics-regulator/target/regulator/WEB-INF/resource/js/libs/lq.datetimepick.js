...
$.fn.lqdatetimepicker = function (options) {
    ...
    return this.each(function () {
        ...
        var _datevalue = $this.val() == '' || $this.val().length != 10? new Date() : new Date($this.val().replace(/-/g, "/"));
        ...
        if(_this.dateType == 'D'){
            ...
            _select_year_t.html("<em>"+_dateyear+"年</em>");
            ...
            _select_month_t.html("<em>"+_datemonth+"月</em>");
...
$.fn.lqdatetimepicker.setDateData = function($this,_obj,_item,_this){
    ...
    if (typeof(_datetime) == 'object'){
        ...
        for (var i=0; i<_datetime.length; i++){
            ...
            if(_this.dateType == 'D'){
                _time.attr('data-value',_this.date.D.month.getFullYear()+'-'+zerobefore((_this.date.D.month.getMonth()+1))+'-'+zerobefore(_time.attr('data-value')))
            }
